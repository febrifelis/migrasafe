...
export const RULES: Rule[] = [
  ...
  },
  {
    ...
    severity: "HIGH", category: "breaking-change", dialect: "all",
    lock: "access-exclusive", rollback: "easy", dataLoss: "none",
    pattern: /\bALTER\s+SCHEMA\s+\S+\s+RENAME\s+TO\b/i,
    message: "ALTER SCHEMA RENAME breaks all objects referencing the old schema name — views, functions, and application queries will fail.",
    suggestion: "Update all references to the old schema name before renaming, or create a search_path alias for the old name.",
  },
  {
    id: "RENAME_TYPE",
    severity: "HIGH", category: "breaking-change", dialect: "postgresql",
    lock: "access-exclusive", rollback: "easy", dataLoss: "none",
    pattern: /\bALTER\s+TYPE\s+\S+\s+RENAME\s+TO\b/i,
    message: "ALTER TYPE RENAME breaks all columns, functions, and application code referencing the old type name.",
    suggestion: "Create a new type with the new name, migrate all references, then drop the old type.",
  },
  {
    id: "RENAME_TYPE_VALUE",
    severity: "HIGH", category: "breaking-change", dialect: "postgresql",
    lock: "access-exclusive", rollback: "easy", dataLoss: "none",
    pattern: /\bALTER\s+TYPE\s+\S+\s+RENAME\s+VALUE\b/i,
    message: "ALTER TYPE RENAME VALUE breaks application code that compares or stores the old enum label.",
    suggestion: "Audit all application code for the old enum value string before renaming. This cannot be rolled back without a new rename.",
  },
  {
    id: "RENAME_COLUMN",
    severity: "HIGH", category: "breaking-change", dialect: "all",
  ...
  // ── Metadata-only entries for visitor-generated issues (no pattern needed) ───
  {
    id: "ADD_PRIMARY_KEY",
    severity: "HIGH", category: "safety", dialect: "all",
    lock: "access-exclusive", rollback: "hard", dataLoss: "none",
    pattern: /\bADD\s+(?:CONSTRAINT\s+\S+\s+)?PRIMARY\s+KEY\b/i,
    message: "ADD PRIMARY KEY acquires ACCESS EXCLUSIVE and validates every row.",
    suggestion: "Create a UNIQUE index CONCURRENTLY first, then use ADD CONSTRAINT ... PRIMARY KEY USING INDEX.",
  },
  {
    id: "ADD_FOREIGN_KEY",
    severity: "HIGH", category: "safety", dialect: "all",
    lock: "row-exclusive", rollback: "hard", dataLoss: "none",
    pattern: /\bADD\s+(?:CONSTRAINT\s+\S+\s+)?FOREIGN\s+KEY\b(?![\s\S]*\bNOT\s+VALID\b)/i,
    message: "ADD FOREIGN KEY without NOT VALID scans the entire table to validate existing rows.",
  ...
  },
  {
    ...
    message: "DROP VIEW breaks all queries, reports, and application code referencing this view.",
    suggestion: "Create a replacement view first, deprecate the old name in application code, then drop.",
  },
  {
    id: "SET_STATEMENT_TIMEOUT_ZERO",
    severity: "HIGH", category: "safety", dialect: "postgresql",
    lock: "none", rollback: "easy", dataLoss: "none",
    pattern: /\bSET\s+(?:LOCAL\s+)?statement_timeout\s*(?:=|TO)\s*(?:'0'|"0"|0)(?:\s|;|$)/i,
    message: "SET statement_timeout = 0 disables all query timeouts — a runaway migration query will hold locks indefinitely.",
    suggestion: "Set a finite statement_timeout (e.g. SET statement_timeout = '30min') so hung queries fail fast.",
  },
  {
    id: "SET_IDLE_TRANSACTION_TIMEOUT_ZERO",
    severity: "HIGH", category: "safety", dialect: "postgresql",
    lock: "none", rollback: "easy", dataLoss: "none",
    pattern: /\bSET\s+(?:LOCAL\s+)?idle_in_transaction_session_timeout\s*(?:=|TO)\s*(?:'0'|"0"|0)(?:\s|;|$)/i,
    message: "SET idle_in_transaction_session_timeout = 0 disables idle-in-transaction cleanup — a stalled connection holds all its locks forever.",
    suggestion: "Set a finite timeout (e.g. SET idle_in_transaction_session_timeout = '60s') to automatically release stalled sessions.",
  },
  {
    id: "ALTER_TYPE_ADD_VALUE",
    severity: "MEDIUM", category: "safety", dialect: "postgresql",
    lock: "access-exclusive", rollback: "irreversible", dataLoss: "none",
    pattern: /\bALTER\s+TYPE\s+\S+\s+ADD\s+VALUE\b/i,
    message: "ALTER TYPE ADD VALUE cannot run inside a transaction block in PostgreSQL < 12 — migration tools that wrap files in BEGIN/COMMIT will fail.",
    suggestion: "Run ALTER TYPE ADD VALUE in a standalone migration file without transaction wrapping, or upgrade to PostgreSQL 12+.",
  },
  {
    ...
    message: "CREATE OR REPLACE VIEW silently changes the view definition — existing queries may break if columns are removed or reordered.",
  ...
  },
  {
    ...
    message: "DROP TRIGGER removes validation or audit logic — data integrity may be bypassed after this.",
    suggestion: "Verify the trigger is truly obsolete and create any replacement before dropping the old one.",
  },
  {
    id: "ATTACH_PARTITION",
    severity: "MEDIUM", category: "performance", dialect: "postgresql",
    lock: "access-exclusive", rollback: "easy", dataLoss: "none",
    pattern: /\bATTACH\s+PARTITION\b/i,
    message: "ATTACH PARTITION acquires a brief ACCESS EXCLUSIVE lock during the metadata update.",
    suggestion: "Add CHECK NOT VALID constraint on the partition first to make the lock phase near-instant.",
  },
  {
    id: "DETACH_PARTITION",
    severity: "HIGH", category: "performance", dialect: "postgresql",
    lock: "access-exclusive", rollback: "easy", dataLoss: "none",
    pattern: /\bDETACH\s+PARTITION\b/i,
    message: "DETACH PARTITION without CONCURRENTLY acquires ACCESS EXCLUSIVE.",
    suggestion: "Use DETACH PARTITION ... CONCURRENTLY (PostgreSQL 14+) to avoid blocking.",
  },
  {
    id: "ALTER_EXTENSION_UPDATE",
    severity: "MEDIUM", category: "breaking-change", dialect: "postgresql",
    lock: "access-exclusive", rollback: "hard", dataLoss: "none",
    pattern: /\bALTER\s+EXTENSION\s+\S+\s+UPDATE\b/i,
    message: "ALTER EXTENSION UPDATE upgrades the extension — function signatures or behavior may change and break callers.",
    suggestion: "Test the extension upgrade in staging. Review the extension changelog for breaking changes before deploying.",
  },
  {
